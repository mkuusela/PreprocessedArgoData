...
for iFile = 1:nFile
    ...
    if profYear < startYear || profYear > endYear
        netcdf.close(fid);
        if mod(iFile,progInterval) == 0
            parfor_progress;
        end
        continue; % Profile didn't occur between 2007-2016
    end

    if profMode == 'D' || profMode == 'A'
    ...
    % Reject profiles with large (> 200 db) pressure jumps
    ...
    badProfile = profPosQCBoolean + sum(profPresQCBoolean) + sum(profTempQCBoolean) + sum(profPsalQCBoolean) + profPresEmpty + profTempEmpty + profPsalEmpty + profPresTempLength + profPresPsalLength + profPresSingleton + profPresInc + profInvalidPos;
    %badProfile = zeros(1,nProf);
...
save('./Data/Argo_data_aggr.mat','startYear','endYear','nProf','profLatAggr','profLongAggr','profYearAggr','profMonthAggr','profFloatIDAggr','profJulDayAggr','profPresAggr','profTempAggr','profPsalAggr','-v7.3');